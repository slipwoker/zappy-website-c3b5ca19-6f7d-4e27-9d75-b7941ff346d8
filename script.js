document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    function getFieldError(field) {
      ...
      if (value !== '') {
        ...
        if ((type === 'text' || field.tagName.toLowerCase() === 'textarea') && value.length < 2) {
    ...
    contactForm.addEventListener('submit', function (e) {
      ...
      if (!isValid) {
        ...
        if (firstError) {
          ...
          const top = firstError.getBoundingClientRect().top + window.pageYOffset - offset - 20;
    ...
    function showFormServerError(submitBtn) {
      let serverErrorEl = contactForm.querySelector('.form-server-error');
      if (!serverErrorEl) {
        serverErrorEl = document.createElement('div');
        serverErrorEl.className = 'form-server-error';
        serverErrorEl.setAttribute('role', 'alert');
        contactForm.insertBefore(serverErrorEl, contactForm.firstChild);
      }
  ...
  const observerTargetClass = 'fade-in-on-scroll';

  const scrollElements = document.querySelectorAll(
    '.fade-in, .animate-on-scroll, [data-animate], .section-title, .card, .feature, .service-item, .team-member, .testimonial, .pricing-card'
  );

  scrollElements.forEach(function (el) {
    el.classList.add(observerTargetClass);
  });

  const style = document.createElement('style');
  style.textContent = [
    '.' + observerTargetClass + '{opacity:0;transform:translateY(24px);transition:opacity 0.6s ease,transform 0.6s ease;}',
    '.' + animationClass + '{opacity:1 !important;transform:translateY(0) !important;}',
  ].join('');
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'c3b5ca19-6f7d-4e27-9d75-b7941ff346d8',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'c3b5ca19-6f7d-4e27-9d75-b7941ff346d8';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'c3b5ca19-6f7d-4e27-9d75-b7941ff346d8';
